...
/**
 * Immutable single-valued ASCII gRPC metadata view. Represents headers as Map<String,
 * Optional<String>>. Assumptions: - Binary keys (ending with "-bin") are ignored.
 */
        ...
        md.keys().stream()
            ...
            .collect(
                Collectors.toMap(
                    ...
                    k ->
                        Optional.ofNullable(
                            md.get(Metadata.Key.of(k, Metadata.ASCII_STRING_MARSHALLER))),
                    (a, b) -> b,
                    LinkedHashMap::new));
    return of(map);
  }
